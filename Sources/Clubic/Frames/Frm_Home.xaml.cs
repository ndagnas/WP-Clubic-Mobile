	...
	public partial class Frm_Home : PhoneApplicationPage, IPhoneApplicationPage
				...
				if ( Args is ObjectEventArgs<SectionType> )
					...
					if ( Arguments != null )
						...
						this.LayoutRoot.CloseSelectedPanel ( (X, Y) =>
							{
							if ( this.PageNavigator.IsLast ( Arguments.Self ) ) this.Navigate ( false, Arguments.Self );
							else                                                this.Navigate ( true , Arguments.Self );
							} );
						//-------------------------------------------------------------------------------------------------------
						}
					//-----------------------------------------------------------------------------------------------------------
		...
		private void ActiveLeftPanel ()
			...
			switch ( Instance.PanelMode )
				...
				case PanelMode.Grid :
					...
					if ( this.LayoutRoot.LeftContent != this.LeftPanelGrid ) this.LayoutRoot.LeftContent = this.LeftPanelGrid;
				...
				case PanelMode.List :
					...
					if ( this.LayoutRoot.LeftContent != this.LeftPanelList ) this.LayoutRoot.LeftContent = this.LeftPanelList;
		...
		private void SetTitle ( SectionType Section )
			...
			switch ( Section )
				...
				case SectionType.Folders    : this.Section.Text = "DOSSIERS";               break;
				//---------------------------------------------------------------------------------------------------------------
				case SectionType.Hardware   : this.Section.Text = "HARDWARE";               break;
				case SectionType.Internet   : this.Section.Text = "INTERNET";               break;
				case SectionType.Mac        : this.Section.Text = "UNIVERS MAC";            break;
				case SectionType.Mobility   : this.Section.Text = "MOBILITÉ ET TÉLÉPHONIE"; break;
				case SectionType.Multimedia : this.Section.Text = "IMAGE ET SON";           break;
				case SectionType.Network    : this.Section.Text = "RÉSEAUX";                break;
				case SectionType.Pro        : this.Section.Text = "CLUBIC PRO";             break;
				case SectionType.Relaxation : this.Section.Text = "DÉTENTE";                break;
				case SectionType.Security   : this.Section.Text = "SECURITÉ INFORMATIQUE";  break;
				case SectionType.Software   : this.Section.Text = "LOGICIEL";               break;
				case SectionType.Technology : this.Section.Text = "TECHNOLOGIE D'AVENIR";   break;
				case SectionType.Tv         : this.Section.Text = "TÉLÉVISION";             break;
				//---------------------------------------------------------------------------------------------------------------
				}
			//-------------------------------------------------------------------------------------------------------------------
			}
		...
		private void OnLoaded ( object Sender, RoutedEventArgs Args )
			...
			if ( ! this.Initialized )
				...
				this.Dispatcher.Async ( () =>
					{
					//-----------------------------------------------------------------------------------------------------------
					if ( ! this.ToastedArticle.IsEmpty ) this.Navigate ( true, this.ToastedArticle );
					else                                 this.Navigate ( true, this.CurrentSection );
				...
				if ( this.NavigationContext != null && this.NavigationContext.QueryString != null 
					...
					if ( this.NavigationContext.QueryString.ContainsKey ( "Section" ) )
						...
						switch ( this.NavigationContext.QueryString["Section"] )
							...
							case "Folders"    : CurrentSection = SectionType.Folders;    break;
							case "Hardware"   : CurrentSection = SectionType.Hardware;   break;
							case "Internet"   : CurrentSection = SectionType.Internet;   break;
							case "Mac"        : CurrentSection = SectionType.Mac;        break;
							case "Mobility"   : CurrentSection = SectionType.Mobility;   break;
							case "Multimedia" : CurrentSection = SectionType.Multimedia; break;
							case "Network"    : CurrentSection = SectionType.Network;    break;
							case "Pro"        : CurrentSection = SectionType.Pro;        break;
							case "Relaxation" : CurrentSection = SectionType.Relaxation; break;
							case "Security"   : CurrentSection = SectionType.Security;   break;
							case "Software"   : CurrentSection = SectionType.Software;   break;
		...
		protected override void OnOrientationChanged ( OrientationChangedEventArgs Args )
			...
			if ( this.LeftPanelPopup != null ) this.LeftPanelPopup.OnOrientationChanged ( Orientation );
			//-------------------------------------------------------------------------------------------------------------------
		...
		private void Navigate ( bool AllowCache, object Source )
			...
			if ( this.PageNavigator.Count > 0 )
				...
				if ( StrScrollTop is string ) this.PageNavigator.Top = StrScrollTop.ToString ().ToInteger ();
				//---------------------------------------------------------------------------------------------------------------
				}
			//-------------------------------------------------------------------------------------------------------------------
		...
		private void NavigateOnComplete ( object Sender, RestEventArgs Args )
			...
			if ( Source is SectionType )
				{
				//---------------------------------------------------------------------------------------------------------------
				switch ( (SectionType)Source )
					{
					//-----------------------------------------------------------------------------------------------------------
					case SectionType.All        : Instance.SendGaView ( Args, "Section", "Toute l'actualité"      ); break;
					case SectionType.Bookmarks  : Instance.SendGaView ( Args, "Section", "Mes Favoris"            ); break;
					case SectionType.Folders    : Instance.SendGaView ( Args, "Section", "Dossiers"               ); break;
					//-----------------------------------------------------------------------------------------------------------
					case SectionType.Hardware   : Instance.SendGaView ( Args, "Section", "Hardware"               ); break;
					case SectionType.Internet   : Instance.SendGaView ( Args, "Section", "Internet"               ); break;
					case SectionType.Mac        : Instance.SendGaView ( Args, "Section", "Univers mac"            ); break;
					case SectionType.Mobility   : Instance.SendGaView ( Args, "Section", "Mobilité et téléphonie" ); break;
					case SectionType.Multimedia : Instance.SendGaView ( Args, "Section", "Image et son"           ); break;
					case SectionType.Network    : Instance.SendGaView ( Args, "Section", "Réseaux"                ); break;
					case SectionType.Relaxation : Instance.SendGaView ( Args, "Section", "Détente"                ); break;
					case SectionType.Security   : Instance.SendGaView ( Args, "Section", "Securité informatique"  ); break;
					case SectionType.Software   : Instance.SendGaView ( Args, "Section", "Logiciel"               ); break;
					case SectionType.Pro        : Instance.SendGaView ( Args, "Section", "Clubic pro"             ); break;
					case SectionType.Technology : Instance.SendGaView ( Args, "Section", "Technologie d'avenir"   ); break;
					case SectionType.Tv         : Instance.SendGaView ( Args, "Section", "Télévision"             ); break;
					//-----------------------------------------------------------------------------------------------------------
					}
				//---------------------------------------------------------------------------------------------------------------
				}
			...
			//-------------------------------------------------------------------------------------------------------------------
				...
				if ( ! string.IsNullOrEmpty ( Article.Title ) )
					{
					if ( Article.IsFolder )
						Instance.SendGaView ( Args, "Dossier", Article.Title, "N°" + Article.ArticleID );
					else
						Instance.SendGaView ( Args, "Article", Article.Title, "N°" + Article.ArticleID );
					}
				else
					{
					if ( Article.IsFolder )
						Instance.SendGaView ( Args, "Dossier", "N°" + Article.ArticleID );
					else
						Instance.SendGaView ( Args, "Article", "N°" + Article.ArticleID );
					}
		...
		private void WebBrowser_LoadCompleted ( object Sender, NavigationEventArgs Args )
			...
			if ( Top > 0 ) this.WebBrowserControl.Eval ( "document.documentElement.scrollTop = " + Top );
			//-------------------------------------------------------------------------------------------------------------------
		...
		private void OnShareButtonClick ( object Sender, EventArgs Args )
			{
			//-------------------------------------------------------------------------------------------------------------------
			Article Article = this.PageNavigator.Last as Article;
			//-------------------------------------------------------------------------------------------------------------------
			...
			if ( Article != null )
				{
				//---------------------------------------------------------------------------------------------------------------
				string Uri   = Article.Uri;
				string Title = Article.Title;

				if ( string.IsNullOrEmpty ( Title ) )
					{
					if ( Article.IsFolder )
						Title = string.Format ( "Dossier n°{0} - clubic.com", Article.ArticleID );
					else
						Title = string.Format ( "Article n°{0} - clubic.com", Article.ArticleID );
					}
				//---------------------------------------------------------------------------------------------------------------
