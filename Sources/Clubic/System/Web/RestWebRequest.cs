...
namespace System.Web
	...
	sealed class RestWebRequest
			...
			try
				...
				if ( this.Files.Count > 0 )
					...
					foreach ( var Item in this.QueryString )
						...
						byte[] formitembytes = this.Encoding.GetBytes ( string.Format 
		...
		//-----------------------------------------------------------------------------------------------------------------------
		private void AsyncComplete ( AsyncRequestState AsyncState, Exception Exception, 
			...
			//-------------------------------------------------------------------------------------------------------------------
				...
				{
				//---------------------------------------------------------------------------------------------------------------
				Exception = new WebException ( "HttpWebResponse is null", WebExceptionStatus.
				                                                                  ReceiveFailure );
				E = new AsyncCompletedEventArgs ( AsyncState.Request    ,
				                                  false                 , 
												  Exception             , 
		...
		public void Async ( AsyncCompletedEventHandler OnComplete, Uri Address, object UserToken )
			...
			Thread AsyncThread = new Thread ( () =>
				...
				this.Async ( new AsyncRequestState ( Address, Request, this.TimeOut, UserToken, 
			                                                                        OnComplete ) );
				} ); 
